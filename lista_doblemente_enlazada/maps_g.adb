...
package body Maps_G is
   ...
   begin
      ...
      if not Found then
         ...
         -- En la lista doblemente enlazada cuando metes el primero su .Prev va a estar a null

         if M.Length = 0 then
            M.P_Last := M.P_First;
         end if;
         -- si la lista esta vacia apuntamos P_Last a P_First(porque solo hay un P)
   ...
   begin
      ...
      while not Success and P_Current /= null  loop
         if P_Current.Key = Key then
            ...
            if M.P_Last = P_Current then
               M.P_Last := M.P_Last.Prev;
            end if;
